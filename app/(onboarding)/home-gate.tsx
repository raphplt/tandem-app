		...
		<OnboardingShell
			...
			subtitle={
				<Trans id="onboarding.homeGate.subtitle">
					Nous récupérons ton profil côté Tandem et préparons ton premier tandem.
				</Trans>
			}
		>
			<View
				className="items-center flex flex-col py-12"
				style={{ gap: 16 }}
			>
				...
				<Text className="text-sm font-body text-typography-500 dark:text-zinc-400">
					{isLoading ? (
						...
						<Trans id="onboarding.homeGate.error">
							Impossible de récupérer ton profil pour le moment. Réessaie dans un instant.
						</Trans>
					) : (
						<Trans id="onboarding.homeGate.success">
							Profil synchronisé, préparation du premier tandem…
						</Trans>
					)}
				</Text>
				{isError ? (
